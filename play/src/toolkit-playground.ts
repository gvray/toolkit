...
class DemoManager {
  ...
  private runValidationkitDemo(demo: string, button: HTMLButtonElement) {
    ...
    switch (demo) {
      ...
      case 'phone': {
        const validationResultPhone = validationkit.phone(value)
        const isValidPhone = validationResultPhone.isValid
      ...
      case 'required': {
        const validationResultRequired = validationkit.required(value)
        const isRequired = validationResultRequired.isValid
