...
describe('validators', () => {
  ...
  describe('idCard', () => {
    it('should validate correct CN ID cards', () => {
      ['11010519491231002X', '110101199001010015', '11010119900101004X'].forEach((id) => {
        expect(idCard(id, 'CN').isValid).toBe(true);
      });
    });

    it('should invalidate incorrect CN ID cards', () => {
      // format failures
      ['123456789012345678', '01010519491231002X', '110105194912310', 'invalid-id'].forEach(
        (id) => {
          expect(idCard(id, 'CN').isValid).toBe(false);
        }
      );
      // format passes but checksum fails
      expect(idCard('110105194912310021', 'CN').isValid).toBe(false);
    });
  });

  describe('personName', () => {
    it('should validate CN names', () => {
      ['张三', '欧阳娜娜', '李小龙'].forEach((n) => {
        expect(personName(n, 'CN').isValid).toBe(true);
      });
    });

    it('should validate CN minority names with middle dot', () => {
      expect(personName('艾力·木沙', 'CN').isValid).toBe(true);
      expect(personName('迪丽热巴·迪力木拉提', 'CN').isValid).toBe(true);
    });

    it('should invalidate bad CN names', () => {
      ['A三', '张', '张三李四王', '张 三'].forEach((n) => {
        expect(personName(n, 'CN').isValid).toBe(false);
        expect(personName(n, 'CN').message).toBe('Invalid name / 姓名格式无效');
      });
    });

    it('should validate generic non-empty names', () => {
      expect(personName('John').isValid).toBe(true);
      expect(personName('').isValid).toBe(false);
    });
  });
  ...
  describe('integer', () => {
    it('should validate signed integer strings', () => {
    ...
    it('should validate unsigned integers with { unsigned: true }', () => {
      expect(integer('42', { unsigned: true }).isValid).toBe(true);
      expect(integer('-1', { unsigned: true }).isValid).toBe(false);
    });
  });
  ...
  describe('bankCard', () => {
    it('should validate CN bank cards (Luhn)', () => {
      expect(bankCard('6222021302120006', 'CN').isValid).toBe(true);
      expect(bankCard('6222021302120006001', 'CN').isValid).toBe(true);
    });

    it('should invalidate cards that fail Luhn or format', () => {
      expect(bankCard('6222021302120003', 'CN').isValid).toBe(false);
      expect(bankCard('1234', 'CN').isValid).toBe(false);
      expect(bankCard('abcd1234567890123', 'CN').isValid).toBe(false);
    });
  });
});
