...
describe('url helpers', () => {
  it('buildUrl combines base, path and query', () => {
    ...
    expect(buildUrl('https://api.com', '/users?active=1', { page: 2 })).toBe(
      'https://api.com/users?active=1&page=2'
    );
  });
  ...
  it('addQueryParam appends a query parameter', () => {
    expect(addQueryParam('https://example.com', 'token', 'abc')).toBe(
  ...
  it('replaceQueryParam replaces a query parameter', () => {
    expect(replaceQueryParam('https://example.com?page=1', 'page', '2')).toBe(
